...
function ShopifyAIPageInner({ urlShopId }: { urlShopId: number | null }) {
  ...
  return (
    <div className="flex flex-col h-screen" style={{ background: "var(--color-bg)" }}>
      <Header title="商品标题及详情优化" subtitle="批量优化商品标题、卖点描述、图片标签和结构化数据" />
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          {showProducts && !isOptimizing && (
            <>
              ...
              <Button onClick={() => setActionPanel(actionPanel === "seo" ? null : "seo")}
                ...
                <Sparkles className="w-4 h-4" />标题与详情优化
              </Button>
        ...
        {actionPanel === "seo" && showProducts && (
          <div className="rounded-xl p-4 border space-y-3"
            style={{ background: "var(--color-card)", borderColor: "var(--color-border)" }}>
            <p className="text-sm font-medium text-white">标题与详情优化 · 已选 {selectedIds.size} 件</p>
            <p className="text-xs text-slate-400">AI 将生成商品标题、卖点描述、图片标签和 Product Schema，预览后确认写入</p>
        ...
        {step === "done" && applyResult && (
          <div className={`rounded-xl p-4 border flex items-center gap-3 text-sm ${
            ...
            <CheckCircle2 className="w-4 h-4 shrink-0" />
            已成功写入 {applyResult.success} 件商品优化内容
