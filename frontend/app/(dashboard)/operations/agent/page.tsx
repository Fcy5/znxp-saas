...
const agentCapabilities = [
  ...
  },
  {
    ...
    badgeVariant: "success" as const,
    status: "live" as const,
    description: "批量为选品库商品生成商品标题、卖点描述、HTML 详情页（含 Q&A 结构化内容），适合站内外搜索和转化。",
  ...
  },
  {
    ...
    badgeVariant: "success" as const,
    status: "live" as const,
    description: "字段映射到 Shopify，图片文件智能命名，GraphQL stagedUploadsCreate 直传媒体库，同步写入图片说明文本。",
    steps: ["字段映射", "图片智能命名", "GraphQL 直传", "图片说明绑定"],
  },
]
...
function ProductPickerModal({ onConfirm, onClose, title = "选择要生成文案的商品", singleSelect = false, shopId: defaultShopId }: {
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      setLoading(true)
      try {
        const r = await productApi.myLibrary(1, 50, keyword || undefined, shopId)
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        {/* Hero Banner */}
        <div className="relative overflow-hidden rounded-[32px] border border-cyan-500/16 panel-soft hero-glow p-7">
          <div className="pointer-events-none absolute inset-0">
            <div className="absolute -left-10 top-0 h-56 w-56 rounded-full bg-cyan-500/12 blur-3xl" />
            <div className="absolute right-0 top-4 h-64 w-64 rounded-full bg-violet-500/10 blur-3xl" />
          </div>
          <div className="relative flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-3xl">
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-2xl font-semibold tracking-tight text-foreground">AI 运营全自动工作流</span>
              </div>
              <p className="text-sm leading-7 text-muted-foreground max-w-2xl">
                从店铺诊脉、跨平台筛品、商品标题与详情优化，到社媒创意、视频二创和 Shopify 上架，整条链路集中在一个运营工作台里完成。
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {agentCapabilities.map((cap) => {
                ...
                return (
                  <Card
                    key={cap.type}
                    className={`group overflow-hidden ${isWip ? "opacity-60" : "card-hover cursor-pointer"}`}
            ...
            {shops.length > 0 && (
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-xs">商品标题及详情优化</CardTitle>
                </CardHeader>
                <CardContent className="pt-0 space-y-1.5">
                  {shops.map(s => (
                    <button
                      key={s.id}
                      onClick={() => router.push(`/seo/shopify-ai?shop_id=${s.id}`)}
